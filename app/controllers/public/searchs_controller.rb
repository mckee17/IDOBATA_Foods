class Public::SearchsController < ApplicationController
  def search
    ...
    when "compound"
      ...
      @search_names = @search.joins(:foods).select(:food_name_id).distinct.pluck(:food_name_id)
      @search_names = FoodName.where(id: @search_names).order(created_at: :desc)
    ...
    when "function"
      ...
      @search_names = @search_foods.select(:food_name_id).distinct.pluck(:food_name_id)
      @search_names = FoodName.where(id: @search_names).order(created_at: :desc)
      @search_count = @search_foods.count
    else
      @search_names = {}
